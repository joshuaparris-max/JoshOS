...
for path in "$launcher" "$acceptance" "$runtime_acceptance" "$runtime_page" "$runtime_download" "$session" "$persist" "$media_probe" "$media_page" "$policy" "$autologin_group_hook" "$live_user_hook" "$lightdm"; do
